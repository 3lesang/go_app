...
func GetCollectionsHandler(c *fiber.Ctx) error {
	...
	results, err := db.ProductQueries.GetCollections(ctx, product_db.GetCollectionsParams{
	...
	total, err := db.ProductQueries.CountCollections(c.Context())
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"error": err.Error(),
		})
	...
	return c.Status(fiber.StatusOK).JSON(PaginatedResponse[product_db.GetCollectionsRow]{
		Page:       page,
		PageSize:   pageSize,
		TotalItems: total,
